...
export default function Banner() {
  ...
  const [seed, setSeed] = useState(Math.floor(Math.random() * 254) + 1);
  const height = bs * breadth;
  let interval;

  const StateLink = ({ setter, num }) => {
    return (
      <span className="hover:text-stone-400" onClick={e => setter(num)}>{num}</span>
    )
  };

  const ValueChanger = ({ name, value, setter, presets }) => {
    return (<p>
      {name}: <input className="bg-stone-300 w-12" type="number" value={value} onChange={e => setter(e.target.value)}/>
  ...
  return (
    <>
      ...
      <div id="banner-info" className="font-serif flex h-full flex-row-reverse" >
        <div className="z-50 bg-stone-300 " hidden={!showInfo}>
          <p>The banner you see here is generated by elementary cellular automata, as described famously by Stephen Wolfram</p>
          <ValueChanger name="rule" value={rule} setter={setRule} presets={[30, 45, 110]} />
          ...
          <p className="hover:text-stone-400" onClick={e => setSeed(Math.floor(Math.random() * Math.pow(2, breadth)))}>random seed</p>
          <p className="hover:text-stone-400" onClick={e => setShowInfo(false)}>hide controls</p>
        </div>
      </div>
    </>
  );
