...
The Self VM pioneered techniques in optimization for dynamic languages, culminating in Urs Hölzle's [runtime type feedback](https://bibliography.selflanguage.org/_static/type-feedback.pdf). Many optimizations would later be put to use in the V8 Javascript VM by fellow Self VM wizard Lars Bak. Later in the project, it was shown that both [Smalltalk and Java](http://merlintec.com/vmworkshop99/sub.pdf) could be implemented in Self, a feat which also happened to produce a much faster Smalltalk implementation.
